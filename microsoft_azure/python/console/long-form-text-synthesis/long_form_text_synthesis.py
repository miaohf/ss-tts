...
class LongTextSynthesizer:
    ...
    def _create_synthesizer(self) -> speechsdk.SpeechSynthesizer:
        ...
        print('_create_synthesizer good ******************************************')
    ...
    def synthesize_text_once(self, text: str) -> Tuple[speechsdk.SpeechSynthesisResult,
        ...
        print('synthesize_text_once good ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^*')
        with self.synthesizer_pool.borrow_synthesizer() as synthesizer:
            ...
            for _ in range(3):  # retry count
                text_boundaries = []
                finished = []
                result = synthesizer.speak_ssml_async(text).get() if self.is_ssml else synthesizer.speak_text_async(text).get()

                if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                    # logger.debug("Synthesis completed %s", text)
                    while not finished:
                        time.sleep(0.1)
                    print('borrow_synthesizer good !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
    ...
    def read_and_split_ssml(ssml_path: Path) -> List[str]:
        ...
        print('read_and_split_ssml good')
        return sentences


if __name__ == "__main__":
    logging.basicConfig(level=logging.INFO)
    YourSubscriptionKey, YourServiceRegion = "f687802b8dbc49d58661816c360e8d43", "eastus"
    print(YourSubscriptionKey, YourServiceRegion)
    ...
    for v in voices_result.voices:
        if v.locale == 'en-US':
            ...
            with Path('./ao_governance_caption.txt').open('r', encoding='utf-8') as r:
                s.synthesize_text(r.read(), output_path=Path('./ao_governance_voice_output'))
